...
module Material
  ...
  module FloatingActionButtonHelper
    ...
    def fab_wrapper(classnames = nil)
      tag.div(class: join_classnames('fab-add', classnames)) { yield }
    end

    def fab_button(path = '#', options = {})
      link_to(path, fab_button_options(path, options)) { material_icon 'add' }
    end

    def fab_button_options(path, options = {})
      options.merge!(class: fab_class('btn-secondary'), role: :button)
    ...
    def fab_menu_item(path, options = {})
      options.merge!(class: fab_class('btn-light btn-sm'), role: :button)
      link_to(path, options) { yield }
    end

    def fab_dropdown_wrapper
      tag.div(class: 'dropdown-menu') { yield }
    end
  end
end
